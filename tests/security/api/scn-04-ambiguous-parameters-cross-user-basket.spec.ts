...
test.describe('SCN-04: duplicate BasketId enables cross-user basket mutation', () => {
    test('evidence: a second BasketId lets User B add an item to User A basket', { tag: '@evidence-pass' }, async ({ newEphemeralApiUser }) => {
        const userA = await newEphemeralApiUser('SCN04-EVIDENCE-A', 'a');
        const userB = await newEphemeralApiUser('SCN04-EVIDENCE-B', 'b');
        const basketApiA = new BasketApi(userA.api, userA.token);
        const basketApiB = new BasketApi(userB.api, userB.token);

        const setup = await test.step('capture each user basket state', async () => {
            const userABefore = await basketApiA.getSnapshotById(userA.basketId, 'User A before exploit');
            const userBBefore = await basketApiB.getSnapshotById(userB.basketId, 'User B before baseline add');
    ...
    test('secure invariant: ambiguous BasketId must not let User B modify User A basket', { tag: '@secure-invariant-fail' }, async ({ newEphemeralApiUser }) => {
        ...
        const userA = await newEphemeralApiUser('SCN04-INVARIANT-A', 'a');
        const userB = await newEphemeralApiUser('SCN04-INVARIANT-B', 'b');
        const basketApiA = new BasketApi(userA.api, userA.token);
        const basketApiB = new BasketApi(userB.api, userB.token);

        const setup = await test.step('capture each user basket state', async () => {
            const userABefore = await basketApiA.getSnapshotById(userA.basketId, 'User A before invariant check');
            const userBBefore = await basketApiB.getSnapshotById(userB.basketId, 'User B before invariant check');
